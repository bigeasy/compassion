...
const Timer = require('happenstance/timer')
...
const Conduit = require('conduit/conduit')
...
class Connection {
    constructor (destructible, colleague, ua) {
...
class Local {
    ...
    _record (destructible, kibitzer, id) {
        destructible.durable('played', new Recorder(this.events, id, 'kibitzer').pump(kibitzer.played))
        destructible.durable('paxos', new Recorder(this.events, id, 'paxos').pump(kibitzer.paxos.outbox))
        destructible.durable('islander', new Recorder(this.events, id, 'islander').pump(kibitzer.islander.outbox))
        return kibitzer
    }

    async _connect (inbox, outbox) {
    ...
    _getColleagueByIslandAndId (island, id) {
    ...
    async _overwatch (colleague, envelope, members, complete) {
        var action
